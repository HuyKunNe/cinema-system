...
### R27.10 Implementation Progress

Completed R27.10 financial-administration HTTP/security boundary:

```text
R27.10.8 — Refund, reconciliation, permissions and audit HTTP boundary — DONE
```

Completed behavior:

- authenticated full-refund HTTP request boundary;
- trusted financial actor identity derived from the validated JWT subject;
- reconciliation resolve and reject HTTP operations;
- financial audit read API using dedicated response DTOs;
- payment:refund authorization;
- payment:reconcile authorization;
- payment:audit authorization;
- explicit isolation between payment:read, payment:refund, payment:reconcile, and payment:audit;
- controller Bean Validation coverage;
- unauthenticated, forbidden and permitted endpoint-security verification;
