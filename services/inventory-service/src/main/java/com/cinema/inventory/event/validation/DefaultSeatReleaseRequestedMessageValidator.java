...
@Component
public class DefaultSeatReleaseRequestedMessageValidator
        implements SeatReleaseRequestedMessageValidator {
    ...
    public void validate(String partitionKey, OutboxEventMessage message) {
        ...
        if (!InventoryEventContract.SEAT_RELEASE_REQUESTED.equals(message.eventType())) {

            throw new ValidationException(InventoryErrorCode.EVENT_TYPE_INVALID);
        }

        if (!InventoryEventContract.SEAT_RELEASE_REQUESTED_VERSION.equals(message.eventVersion())) {
        ...
        requireUuidV7(
                message.causationId(),
                InventoryErrorCode.EVENT_CAUSATION_ID_INVALID,
                InventoryErrorCode.EVENT_CAUSATION_ID_INVALID);
