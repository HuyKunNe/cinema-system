...
@Component
public class DefaultBookingExpiredMessageValidator implements BookingExpiredMessageValidator {
    ...
    public void validate(String partitionKey, OutboxEventMessage message) {
        ...
        if (!InventoryEventContract.BOOKING_EXPIRED.equals(message.eventType())) {

            throw new ValidationException(InventoryErrorCode.EVENT_TYPE_INVALID);
        }

        if (!InventoryEventContract.BOOKING_EXPIRED_VERSION.equals(message.eventVersion())) {
