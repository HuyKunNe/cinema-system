...
@Component
public class DefaultBookingConfirmedMessageValidator implements BookingConfirmedMessageValidator {
    ...
    public void validate(String partitionKey, OutboxEventMessage message) {
        ...
        if (!InventoryEventContract.BOOKING_CONFIRMED.equals(message.eventType())) {

            throw new ValidationException(InventoryErrorCode.EVENT_TYPE_INVALID);
        }

        if (!InventoryEventContract.BOOKING_CONFIRMED_VERSION.equals(message.eventVersion())) {
