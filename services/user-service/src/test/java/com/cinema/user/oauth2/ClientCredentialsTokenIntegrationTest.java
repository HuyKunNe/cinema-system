...
class ClientCredentialsTokenIntegrationTest extends AbstractMySqlIntegrationTest {
    ...
    void registerServiceClient() {
        ...
        assertThat(UUID.fromString(REGISTERED_CLIENT_ID).version()).isEqualTo(7);
    ...
    @Test
    void shouldIssueSignedJwtForClientCredentials() throws Exception {

        MvcResult result =
                requestToken(CLIENT_ID, RAW_CLIENT_SECRET, "inventory:read inventory:write")
        ...
        assertThat(response.has("refresh_token"))
                .as("Client Credentials must not receive a refresh token")
                .isFalse();

        String tokenValue = response.required("access_token").asText();

        Jwt jwt = jwtDecoder.decode(tokenValue);

        assertServiceToken(jwt, CLIENT_ID);
    }

    @Test
    void shouldRejectIncorrectClientSecret() throws Exception {

        requestToken(CLIENT_ID, "incorrect-secret", "inventory:read")
    ...
    @Test
    void shouldRejectUnregisteredScope() throws Exception {

        requestToken(CLIENT_ID, RAW_CLIENT_SECRET, "user:admin")
    ...
    @Test
    void deactivatedClientShouldBeRejectedAsInvalidClient() throws Exception {

        ClientFixture fixture = registerIsolatedServiceClient("deactivate");

        requestToken(fixture.clientId(), fixture.rawSecret(), "inventory:read")
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.access_token").isNotEmpty());

        clientLifecycleService.deactivate(fixture.clientId());
        ...
        assertThat(registeredClientRepository.findByClientId(fixture.clientId())).isNull();
    }

    @Test
    void rotatedSecretShouldRejectOldSecretAndAcceptNewSecret() throws Exception {

        ClientFixture fixture = registerIsolatedServiceClient("rotate");

        requestToken(fixture.clientId(), fixture.rawSecret(), "inventory:read")
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.access_token").isNotEmpty());

        clientLifecycleService.rotateSecret(fixture.clientId(), ROTATED_CLIENT_SECRET);
        ...
        Jwt jwt = jwtDecoder.decode(response.required("access_token").asText());

        assertServiceToken(jwt, fixture.clientId());
    }

    private org.springframework.test.web.servlet.ResultActions requestToken(
            String clientId, String rawSecret, String scopes) throws Exception {

        return mockMvc.perform(
                post("/oauth2/token")
                        .with(httpBasic(clientId, rawSecret))
                        .contentType(MediaType.APPLICATION_FORM_URLENCODED)
                        .param("grant_type", "client_credentials")
                        .param("scope", scopes));
    }

    private ClientFixture registerIsolatedServiceClient(String prefix) {

        String suffix = UUID.randomUUID().toString();

        String registeredClientId = UUID.randomUUID().toString();

        String clientId = "r25-11-8-5-" + prefix + "-" + suffix;

        String rawSecret = "isolated-" + prefix + "-secret";
        ...
        RegisteredClient client =
                serviceClient(
                        ...
                        clientId,
                        rawSecret,
                        "R25.11.8.5 " + prefix + " Client");

        registeredClientRepository.save(client);

        return new ClientFixture(clientId, rawSecret);
    }

    private RegisteredClient serviceClient(
            String registeredClientId, String clientId, String rawSecret, String clientName) {
    ...
    private void assertServiceToken(Jwt jwt, String expectedSubject) {
        ...
        assertThat(jwt.getClaims()).doesNotContainKeys("username", "roles");
    ...
    private record ClientFixture(String clientId, String rawSecret) {}
}
