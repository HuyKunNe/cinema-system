...
class AuthorizationCodePkceTokenIntegrationTest extends AbstractMySqlIntegrationTest {
    ...
    private static final String CODE_VERIFIER =
            "cinema-authorization-code-pkce-verifier-12345678901234567890";
    ...
    @Test
    void shouldIssueUserJwtThroughAuthorizationCodeWithPkce() throws Exception {
    ...
    @Test
    void shouldRejectIncorrectPkceVerifier() throws Exception {
        ...
        MockHttpSession session = login(user);
        String code = authorize(session, createCodeChallenge(CODE_VERIFIER));
        ...
        mockMvc.perform(
                        post("/oauth2/token")
                                ...
                                .param("code_verifier", CODE_VERIFIER + "-incorrect"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.error").value("invalid_grant"));
    }

    @Test
    void shouldRejectMissingPkceVerifierAsInvalidClient() throws Exception {
    ...
    @Test
    void shouldRejectAuthorizationCodeWithMismatchedRedirectUri() throws Exception {
        ...
        mockMvc.perform(
                        post("/oauth2/token")
                                ...
                                .param("client_id", CLIENT_ID)
                                .param("code", code)
                                .param("redirect_uri", "http://127.0.0.1:3000/other-callback")
    ...
    @Test
    void shouldRejectAuthorizationCodeReplay() throws Exception {
    ...
    private void assertPersistedClient(RegisteredClient expectedClient) {

        RegisteredClient persistedClient = registeredClientRepository.findByClientId(CLIENT_ID);

        assertThat(persistedClient).isNotNull();
        assertThat(persistedClient.getId()).isEqualTo(expectedClient.getId());
        assertThat(UUID.fromString(persistedClient.getId()).version()).isEqualTo(7);
        assertThat(persistedClient.getRedirectUris()).containsExactly(REDIRECT_URI);
        assertThat(persistedClient.getScopes())
                .containsExactlyInAnyOrder("booking:read", "booking:create");
        ...
        assertThat(persistedClient.getClientSettings().isRequireProofKey()).isTrue();
        assertThat(persistedClient.getClientSettings().isRequireAuthorizationConsent()).isTrue();
    }
    ...
    private String authorize(MockHttpSession session, String codeChallenge) throws Exception {
        ...
        assertThat(redirect.getQueryParams().getFirst("state")).isEqualTo("test-state");
    ...
    private JsonNode exchangeCode(String code, String codeVerifier) throws Exception {
