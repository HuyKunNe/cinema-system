...
class PaymentFailedKafkaListenerTest {
    ...
    private static final OffsetDateTime FAILED_AT = OffsetDateTime.parse("2026-09-15T10:00:00Z");

    @Mock private PaymentResultMessageReader messageReader;

    @Mock private PaymentFailedConsumerService consumerService;

    private PaymentFailedKafkaListener listener;

    @BeforeEach
    void setUp() {

        listener = new PaymentFailedKafkaListener(messageReader, consumerService);
    }
    ...
    void messageShouldBeReadBeforeConsumerServiceIsCalled() {
        ...
        when(consumerService.handle(partitionKey, message))
                .thenReturn(PaymentFailedConsumerService.Result.paymentFailed());
    ...
    void duplicateResultShouldBeAcceptedWithoutThrowing() {
        ...
        when(consumerService.handle(partitionKey, message))
                .thenReturn(PaymentFailedConsumerService.Result.alreadyProcessed());

        listener.consume(partitionKey, SERIALIZED_MESSAGE);

        verify(messageReader).read(SERIALIZED_MESSAGE);
