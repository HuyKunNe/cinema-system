...
@Component
public class DefaultSeatReservationRejectedMessageValidator
        implements SeatReservationRejectedMessageValidator {
    ...
    public void validate(String partitionKey, OutboxEventMessage message) {
        ...
        if (!BookingEventContract.SEAT_RESERVATION_REJECTED.equals(message.eventType())) {

            throw new ValidationException(BookingErrorCode.EVENT_TYPE_INVALID);
        }

        if (!BookingEventContract.SEAT_RESERVATION_REJECTED_VERSION.equals(
